Log("Loading modified Alien.lua for NS2 Balance Beta mod.")

-- Changes:
-- Adding a check for an optional method on Ability-type classes to get a multiplier for attack
--      speed.
...
local networkVars = 
{
    ...
    abilityEnergyOnChange = "compensated float (0 to " .. math.ceil(kAdrenalineAbilityMaxEnergy) .. " by 0.05 [] )",
...
function Alien:GetLifeformMaxAdrenalineEnergy()
    return kAdrenalineAbilityMaxEnergy
end

function Alien:GetMaxEnergy()
    return ConditionalValue(self.hasAdrenalineUpgrade, (self:GetLifeformMaxAdrenalineEnergy() - kAbilityMaxEnergy) * (self:GetSpurLevel() / 3) + kAbilityMaxEnergy, kAbilityMaxEnergy)
end

function Alien:GetAdrenalineMaxEnergy()

    if self.hasAdrenalineUpgrade then
        return (self:GetLifeformMaxAdrenalineEnergy() - kAbilityMaxEnergy) * (self:GetSpurLevel() / 3)
    end
...
function Alien:OnUpdateAnimationInput(modelMixin)
    ...
    if weapon and weapon.GetAttackSpeedMultiplier then
        attackSpeed = attackSpeed * weapon:GetAttackSpeedMultiplier()
    end
