...
function Order:OnEntityChange(oldId, newId)
    ...
    if self.orderType ~= kTechId.Build and self.orderParam == oldId then
        ...
        if newId == nil then
            ...
            local oldTarget = newTarget and Shared.GetEntity(oldId)
            local oldTeam = newTarget and newTarget.GetTeamNumber and newTarget:GetTeamNumber()
            local newTeam = oldTarget and oldTarget.GetTeamNumber and oldTarget:GetTeamNumber()
...
function GetOrderTargetIsWeldTarget(order, doerTeamNumber)
    ...
    if entityId < 1 then return end -- invalid entId

    local entity = Shared.GetEntity(entityId)
    if not entity then return end -- invalid ent

    if HasMixin(entity, "Weldable") and entity:GetTeamNumber() == doerTeamNumber then
        return entity
    end
end
