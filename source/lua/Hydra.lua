Log("Loading modified Hydra.lua for NS2 Balance Beta mod.")

-- Changes:
-- Skips defining Hydra:GetBarrelPoint() so Hydra_Server can make its own definition.
...
if Server then
    function Hydra:GetEyePos()
        return self.lastHitLocation or (self:GetOrigin() + self:GetViewOffset())
    end
else
    function Hydra:GetEyePos()
