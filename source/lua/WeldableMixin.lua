Log("Loading modified WeldableMixin.lua for NS2 Balance Beta mod.")

-- Changes:
-- Can self-weld if enabled with a method (to allow for exo auto-repair).
-- Added an override parameter for amount to weld per second (why the hell isn't this ALWAYS a parameter?)
...
WeldableMixin.optionalCallbacks =
{
    ...
    OnWeldOverride = "When welded (welding entity, elapsed time, weldPerSecOverride). Returns boolean indicating if weld had effect.",
    GetWeldPercentageOverride = "Returns the weld progress, 0-1 scalar.",
    GetCanSelfWeld = "Returns true if self can be welded.",
}
...
function WeldableMixin:GetCanBeWelded(doer)
    ...
    -- Can't weld yourself (unless you can...)
    if doer == self and not (self.GetCanSelfWeld and self:GetCanSelfWeld()) then
        return false
    end
