Log("Loading modified WebGhostModel.lua for NS2 Balance Beta mod.")
...
function WebGhostModel:Update()
    ...
    local webStalkColor = (valid and Color(1,1,1,1) or Color(0.3,0,0,1))

    if self.desiredVisibility and modelCoords and lastClickedPos then
        ...
        DynamicMesh_SetTwoSidedLine(self.webRenderModel, modelCoords, 0.1, length, webStalkColor, webStalkColor)

    end
    
    self.webRenderModel:SetIsVisible(self.desiredVisibility and (lastClickedPos ~= nil))
