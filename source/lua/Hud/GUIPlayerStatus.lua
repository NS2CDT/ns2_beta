...
-- Call this class inside of a HUD file to add status icons.
-- ParameterNumbers references variable names inside of the relevant HUD file which references functions from Player_Client.lua
-- There is a 'Simple' status icon which does not have a timer, and a status icon with a timer
--
...
GUIPlayerStatus.kBlightTextureCoords = GetTextureCoordinates(1, 10)
...
local function CreateStatusIndicator(self, settings)
    ...
    if settings.EffectIconCoords then
        ...
        statusEffectIcon:SetColor(settings.Color)
...
function GUIPlayerStatus:Reset(scale)
    ...
    self.statusBackground:SetPosition(GUIPlayerStatus.kBackgroundPos)
...
local function StatusTimerLogic(self, parameters)
    for i, bar in ipairs(self.statusIcons) do
        if not bar.Settings.Simple and bar.Settings.Name and table.icount(bar.Settings.ParameterNumbers) == 2 then
            ...
            if self.lastStatusState[bar.Settings.Name] ~= statusState then
                ...
                if self.lastStatusState[bar.Settings.Name] < statusState then
                    if not bar.Settings.NoSizeAnimations and not bar.Settings.NoSizeAnimations == true then
            ...
            if bar.Settings.ShowWithHintsOnly == true then
...
local function SimpleStatusLogic(self, parameters)
...
function GUIPlayerStatus:Update(deltaTime, parameters)

    numActive = 0

    ClearAllStatuses(self)

    StatusTimerLogic(self, parameters)

    SimpleStatusLogic(self, parameters)
    ...
    if lastNumActive ~= numActive then
