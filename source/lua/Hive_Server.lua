...
function Hive:UpdateResearch()
    ...
    if researchId == kTechId.ResearchBioMassOne or researchId == kTechId.ResearchBioMassTwo
            or researchId == kTechId.ResearchBioMassThree then
...
local function FireImpulses(self)
    ...
    if now - self.lastImpulseFireTime > kImpulseInterval then
        ...
        for i = 1, #removals do
            local id = removals[i]
            table.removevalue(self.cystChildren, id)
        end
...
function Hive:AddChildCyst(child)
    table.insert(self.cystChildren, child:GetId())
end
